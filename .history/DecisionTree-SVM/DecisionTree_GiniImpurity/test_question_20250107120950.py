...
class DecisionTreeClassifierGini:
    ...
    def calculate_gini_split(self, y, left_indices, right_indices):
        # your code here
        n = len(y)
        lsum = np.sum(left_indices) 
        rsum = np.sum(right__indices)    
        if(lsum == 0 or rsum == 0)return float('inf')

        gini_left = np.self.gini_impurity(self,left_indices)
        gini_right = np.self.gini_impurity(self,right_indices)
        gini = (lsum/n)*gini_left + (rsum/n)*gini_right
    ...
    def find_best_split(self, X, y):
        ...
        for f in range(X.shape[1]):
            ...
            for t in threshold:
                left_ind,right_ind = self.split(X_column,t)
                gini = self.calculate_gini_split(y,left_ind,right_ind)
