...
def train_svm(X, y, learning_rate=0.01, epochs=1000, lambda_param=0.01):
    ...
    for i in range(epochs):
        for s,f in enumerate(X):
            condition = Y[s] * (np.dot(f,weights) - bias) >= 1
            if condition:
                weights -= learning_rate*(2*lambda_param*weights)
            else:
                weights -= learning_rate*(2*lambda_param*weights - np.dot(f,Y[s]))
                bias -= learning_rate*Y[s]
...
def predict(X, weights, bias):
    pd = np.sign(np.dot(X,weights) - bias)
    return pd
    # your code here
...
def main():
    ...
    labels = labels.apply(lambda a:-1 if a == 0 else 1)
